...
RectangleMesh2D::RectangleMesh2D(int numNodesX, int numNodesY, double size_x,
                                 double size_y)
{
    // std::cout<<"In RectangleMesh2D(int numNodesX, int numNodesY, double size_x,
    // double size_y){\n ";
...
{
    // std::cout<<"RectangleMesh2D() numNodesX = 10; numNodesY = 10; size_x = 1;
    // size_y = 1\n ";
    numNodes['x'] = 10;
    numNodes['y'] = 10;
    size['x'] = 1;
...
void RectangleMesh2D::saveMesh(std::string name_file)
{
    ...
    for (int i = 0; i < numNodes['x']; ++i) {
        for (int j = 0; j < numNodes['y']; ++j) {
            ...
            outFile << std::setw(5) << nodesPosition[id][0] << std::setw(5)
                    << nodesPosition[id][1] << std::setw(5) << id << ";      ";
        }
        outFile << std::endl;
        for (int j = 0; j < numNodes['y']; ++j) {
            int id = numNodes['y'] * i + j;
            outFile << std::setw(3) << elementsId[id][0] << std::setw(3)
                    << elementsId[id][1] << std::setw(3) << elementsId[id][2]
                    << std::setw(3) << elementsId[id][3] << ";      ";
        }
        outFile << std::endl;
        outFile << std::endl;
    }
