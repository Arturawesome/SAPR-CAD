...
LaxWendroffMethod1D::LaxWendroffMethod1D(
    std::function<double(double)> leftBC, std::function<double(double)> rightBC,
    std::function<double(double)> initialC, std::vector<double>& u,
    std::vector<double>& uNext, std::vector<double>& uPrev,
    std::shared_ptr<Mesh1D> meshx, std::shared_ptr<Mesh1D> meshT,
    double courantNumber)
    : leftBC_(leftBC), rightBC_(rightBC), initialC_(initialC), u_(u),
      uNext_(uNext), uPrev_(uPrev), meshx_(std::move(meshx)),
      meshT_(std::move(meshT)), courantNumber_(courantNumber)
...
void LaxWendroffMethod1D::numericalScheme(int ts)
{
    ...
    for (int i = 1; i < sizeMeshx_ - 1; ++i) {
        uNext_[i] = u_[i] - courantNumber_ * (u_[i + 1] - u_[i - 1]) / 2 +
                    courantNumber_ * courantNumber_ * (u_[i + 1] - 2 * u_[i] + u_[i - 1]) / 2;
    }
